...
setPosBegin	jp	_setPosBegin		; позиционировать на начало файла
...
eSearch		jp	_eSearch		; короткая запись для поиска файлов
						; o: Z - файл не найден

storeFileLen	jp	_storeFileLen		; Установить длину файла
						; i: HL,DE - длина

getPanelStatus	jp	_getPanelStatus		; Получить состояние панелей Wild Commander
						; o: a - значение
...
editInit	jp	_editInit		; Начальная инициализация строки ввода

printWW		jp	_printWW		; Обновление (?) строки ввода (Входящих/Исходящих параметров нет)

printInLine	jp	_printInLine		; Печать строки в строку ввода
						; i: HL - адрес строки, заканчивающейся #00 

printRestore	jp	_printRestore		; Печать строки, восстаноавливающей параментры по умолчанию	

int2str		jp	_int2str		; Преобразование десятичного беззнакового числа Int16(16бит) в строку
						; i: HL - чисто
						;    DE - адресс строки куда сохранять (5 байт)

char2str	jp	_char2str		; Преобразование десятичного беззнакового числа Сhar(8bit) в строку
						; i: H=0, L - чисто
						;    DE - адресс строки куда сохранять (3 байта)

fourbit2str	jp	_fourbit2str		; Преобразование десятичного беззнакового числа (4bit) в строку
						; i: H=0, L - чисто
						;    DE - адресс строки куда сохранять (2 байта)

chToHomePath	jp	_chToHomePath		; Установить «домашнюю» директорию как текущую (для загрузки ресурсов приложения)
chToCallPath	jp	_chToCallPath		; Восстановить директорию от куда была вызвана загрузка приложения

checkCallKeys	jp	_checkCallKeys		; Вызов подпрограмм по данным из таблицы
						; i: DE - адресс таблицы с данными
						; o: a = #ff - ошибка

checkIsPath	jp	_checkIsPath		; 

prepareEntry	jp	_prepareEntry		;

setCliResol	jp	_setCliResol		; Reserved

setCliGfxResol	jp	_setCliGfxResol		; Установить разрешение экрана в графическом режиме (активируется по ALT+F2)
						; i:A - видео режим:
						;   #00 - 256x192
						;   #01 - 320x200
						;   #02 - 320x240
...
setAppCallBack	jp	_setAppCallBack		; Установить адресс вызова при переключении ALT+F1/F2 (при вызове в A= 0-txt режим, 1-gfx)
						; i: HL - адресс подпрограммы приложения
